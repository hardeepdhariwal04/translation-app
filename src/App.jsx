...
// const supabaseKey = import.meta.env.VITE_SUPABASE_KEY; // Set in your .env file
...
//     .from('translations')
//     .insert([
//       { original_message: original, translated_message: translated, language, model },
//     ]);
      ...
      <form onSubmit={handleOnSubmit}>
        ...
        >
          {Object.keys(supportedLanguages).map((model) => (
            supportedLanguages[model].map((lang) => (
        ...
        <button type="submit" disabled={isLoading}>
          {isLoading ? <BeatLoader color="#fff" size={8} /> : "Translate"}
        </button>
      </form>
      {error && <p className="error">{error}</p>}
      {showNotification && <p className="notification">Copied to clipboard!</p>}
      {translation && (
